import bcrypt
from fastapi import FastAPI, HTTPException, UploadFile, File, Form, Query, UploadFile
...
async def upload_any_file(file: UploadFile = File(...)):
    ...
    if not ext:
        ext = ".wav, .mp3"  # or any default extension you prefer
        filename = filename + ext

    if await db.file_exists(filename):
        raise HTTPException(status_code=409, detail="File with this name already uploaded")
    
    metadata = await parse_call_filename(filename)
